 ...
 * - Extract LPCM PES payload from TS PID 0x1100 to an optional audio output.
 *
...
static unsigned long long g_lpcm_bytes;
...
static void process_ts_packet(const uint8_t *ts)
{
    ...
    if (pid == VIDEO_PID) {
        ...
        g_audio_ts_packets++;
        write_lpcm_from_ts_payload(ts + off, TS_SIZE - off, payload_start);
    }
}
...
int main(int argc, char **argv)
{
    if (argc < 2) {
        fprintf(stderr,
                "usage: %s <source-ip> [h264-output-file] [lpcm-output-file]\n",
                argv[0]);
        return 2;
    }

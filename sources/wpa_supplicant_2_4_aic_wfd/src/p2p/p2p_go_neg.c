	...
	if (status || peer == NULL) {
		...
		p2p_buf_add_channel_list(buf, p2p->cfg->country,
			...
			p2p_channels_intersect(&p2p->channels, &peer->channels,
					       &res);
			if ((res.reg_classes == 0 ||
			     res.reg_class[0].channels == 0) &&
			    p2p->go_intent == 15) {
				p2p_dbg(p2p, "No common channels in GO Neg Response; advertise local 2.4 GHz channels for RTL8723BU");
				p2p_buf_add_channel_list(buf, p2p->cfg->country,
							 &p2p->channels);
			} else {
				p2p_buf_add_channel_list(buf, p2p->cfg->country,
			  ...
			  u8 *status)
{
	...
	if (intersection.reg_classes == 0 ||
		...
		if (p2p->go_intent == 15) {
			p2p_dbg(p2p, "No common channels found; keep local 2.4 GHz GO channel for RTL8723BU");
			return 0;
		} else {
			    ...
			    const u8 *data, size_t len, int rx_freq)
{
	...
	if (dev && dev->flags & P2P_DEV_USER_REJECTED) {
		...
		if (p2p_peer_channels(p2p, dev, msg.channel_list,
			...
			if (p2p->go_intent == 15) {
				p2p_dbg(p2p, "No common channels found; force local 2.4 GHz GO channel for RTL8723BU");
			} else {
				p2p_dbg(p2p, "No common channels found");
				status = P2P_SC_FAIL_NO_COMMON_CHANNELS;
				goto fail;
					     ...
					     const u8 *resp_chan, int go)
{
	...
	if ((res.reg_classes == 0 || res.reg_class[0].channels == 0) &&
	    p2p->go_intent == 15) {
		p2p_dbg(p2p, "No common channels in GO Neg Confirm; advertise local 2.4 GHz channels for RTL8723BU");
		p2p_buf_add_channel_list(buf, p2p->cfg->country, &p2p->channels);
	} else {

...
static unsigned long long g_h264_dropped_bytes;
...
static void setup_output_fd(const char *path)
{
    struct stat st;
    ...
    if (path) {
        ...
        g_output_fd = open(path, flags, 0666);
        if (g_output_fd < 0)
            die("open output failed: %s: %s", path, strerror(errno));
    } else if (fstat(STDOUT_FILENO, &st) == 0 && S_ISFIFO(st.st_mode)) {
        g_output_nonblock = 1;
        set_nonblock(STDOUT_FILENO);
    }

    if (g_output_nonblock) {
        int pipe_sz = 256 * 1024;
        fcntl(g_output_fd, F_SETPIPE_SZ, pipe_sz);
        fprintf(stderr, "output fifo/pipe guard enabled fd=%d pipe_sz=%d\n",
                g_output_fd, pipe_sz);
    }
}

static void write_output_guarded(const uint8_t *buf, size_t len)
{
    size_t off = 0;

    while (off < len && !g_stop) {
        ssize_t n = write(g_output_fd, buf + off, len - off);
        if (n > 0) {
            off += (size_t)n;
            g_h264_bytes += (unsigned long long)n;
            continue;
        }
        if (n < 0 && errno == EINTR)
            continue;
        if (g_output_nonblock && n < 0 &&
            (errno == EAGAIN || errno == EWOULDBLOCK)) {
            g_h264_dropped_bytes += (unsigned long long)(len - off);
            return;
        }
        if (n < 0 && errno == EPIPE) {
            fprintf(stderr, "output pipe closed\n");
            g_stop = 1;
            return;
        }
                                       ...
                                       int payload_start)
{
    ...
    if (payload_len > off) {
        ...
        write_output_guarded(payload + off, want);
    }
}
...
int main(int argc, char **argv)
{
    ...
    if (argc >= 3) {
        if (!strcmp(argv[2], "/dev/null"))
            g_skip_ts_parse = 1;
        setup_output_fd(argv[2]);
    } else {
        setup_output_fd(NULL);
    }
    ...
    fprintf(stderr,
            "final stats: rtp_packets=%lu rtp_bytes=%llu ts_packets=%lu video_ts_packets=%lu h264_bytes=%llu dropped_bytes=%llu\n",
            rtp_packets, rtp_bytes, g_ts_packets, g_video_ts_packets,
            g_h264_bytes, g_h264_dropped_bytes);

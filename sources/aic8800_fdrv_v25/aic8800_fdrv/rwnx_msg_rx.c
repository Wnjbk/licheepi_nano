 ...
 *
 */
static unsigned int rwnx_msg_hdlr_count(lmac_task_id_t task)
{
    switch (task) {
    case TASK_MM:
        return ARRAY_SIZE(mm_hdlrs);
    case TASK_DBG:
        return ARRAY_SIZE(dbg_hdlrs);
#ifdef CONFIG_RWNX_FULLMAC
    case TASK_TDLS:
        return ARRAY_SIZE(tdls_hdlrs);
    case TASK_SCANU:
        return ARRAY_SIZE(scan_hdlrs);
    case TASK_ME:
        return ARRAY_SIZE(me_hdlrs);
    case TASK_SM:
        return ARRAY_SIZE(sm_hdlrs);
    case TASK_APM:
        return ARRAY_SIZE(apm_hdlrs);
    case TASK_MESH:
        return ARRAY_SIZE(mesh_hdlrs);
#endif
    default:
        return 0;
    }
...
void rwnx_rx_handle_msg(struct rwnx_hw *rwnx_hw, struct ipc_e2a_msg *msg)
{
    ...
    if (msg->param_len > RWNX_CMD_E2AMSG_LEN_MAX) {
        if (printk_ratelimit())
            printk("aicwf: clamp bad e2a msg id=0x%04x param_len=%u max=%u\n",
                   msg->id, msg->param_len, RWNX_CMD_E2AMSG_LEN_MAX);
        msg->param_len = RWNX_CMD_E2AMSG_LEN_MAX;
    }

    task = MSG_T(msg->id);
    idx = MSG_I(msg->id);
    count = rwnx_msg_hdlr_count(task);
    if (count && task < ARRAY_SIZE(msg_hdlrs) && msg_hdlrs[task] && idx < count) {
        cb = msg_hdlrs[task][idx];
    } else if (printk_ratelimit()) {
        printk("aicwf: e2a msg no handler id=0x%04x task=%u idx=%u count=%u\n",
               msg->id, task, idx, count);
    }

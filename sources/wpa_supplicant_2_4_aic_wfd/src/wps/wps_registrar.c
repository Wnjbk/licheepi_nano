			      ...
			      const u8 *addr, const u8 *uuid_e)
{
	...
	/* F1C200S/AIC WFD host test: crowded Miracast environments can
	 * produce unrelated PBC Probe Requests while the phone is connecting.
	 * Keep PBC usable for diagnostics instead of aborting with overlap. */
	if (count > 1)
		wpa_printf(MSG_INFO, "WPS: ignore PBC overlap for F1C200S AIC WFD test");
	return 0;
}

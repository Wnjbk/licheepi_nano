				 ...
				 struct spi_transfer *tfr)
{
	...
	if (tfr->rx_buf) {
		...
		if (dmaengine_slave_config(master->dma_rx, &rxconf))
			return -EINVAL;
	...
	if (tfr->tx_buf) {
		...
		if (dmaengine_slave_config(master->dma_tx, &txconf)) {
			if (rxdesc)
				  ...
				  struct spi_transfer *tfr)
{
	...
	if (use_dma) {
		...
		/*
		 * Setup FIFO interrupt trigger level. Here we choose 3/4 of
		 * the full fifo depth, as used by older Allwinner SPI blocks.
		 */
	...
	if (!use_dma)
		sun6i_spi_enable_interrupt(sspi, SUN6I_INT_CTL_RF_RDY);
	if (!use_dma && tx_len > sspi->fifo_depth)
	...
	if (use_dma && tfr->rx_buf) {
		timeout = wait_for_completion_timeout(&sspi->dma_rx_done,
						      msecs_to_jiffies(tx_time));
		if (!timeout) {
			dev_warn(&master->dev, "%s: timeout waiting for RX DMA",
				 dev_name(&spi->dev));
			ret = -ETIMEDOUT;
			goto out;
		}
			      ...
			      struct spi_transfer *tfr)
{
	...
	/* Keep commands, reads, and tiny transfers on the proven PIO path. */
...
static int sun6i_spi_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(master->dma_rx)) {
		...
		dev_warn(&pdev->dev, "RX DMA unavailable, using TX-only DMA/PIO\n");
		master->dma_rx = NULL;
	}

	if (master->dma_tx) {
		sspi->dma_addr_tx = res->start + SUN6I_TXDATA_REG;
		sspi->dma_addr_rx = res->start + SUN6I_RXDATA_REG;

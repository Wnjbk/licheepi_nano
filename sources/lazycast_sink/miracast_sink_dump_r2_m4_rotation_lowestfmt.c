...
int main(int argc, char **argv)
{
    ...
    snprintf(m3_body, sizeof(m3_body),
        ...
        "wfd2_video_stream_control: 0f 0f 03\r\n",
    ...
    if (getenv("WFD_R2_M4_ROTATE")) {
        const char *m4_body = "wfd2_rotation_control: Landscape_to_Portrait\\r\\n";
        fprintf(stderr, "reply M4 rotation control: %s", m4_body);
        rtsp_parameters_ok_same_cseq(rtsp, buf, m4_body);
    } else {
    ...
    if (getenv("WFD_R2_M21_ROTATE_DISABLED")) {
        const char *m21_body = "wfd2_video_stream_control: 00 00 01\r\n";
        fprintf(stderr, "send M21 video stream control request: %s", m21_body);
        ...
        if (strstr(buf, "RTSP/1.0 200 OK"))
            fprintf(stderr, "M21 video stream control accepted\n");
        else
            fprintf(stderr, "M21 video stream control unexpected response\n");
    }

...
static void aicwf_usb_rx_complete(struct urb *urb)
{
    ...
    if (usb_dev->state == USB_UP_ST) {
        ...
        if (skb->len >= 3) {
            u32 early_pkt_len = skb->data[0] | (skb->data[1] << 8);
            if (early_pkt_len > skb->len) {
                if (printk_ratelimit())
                    printk("aicwf: early drop bad rx skb pkt_len=%u skb_len=%u\n", early_pkt_len, skb->len);
                aicwf_dev_skb_free(skb);
                aicwf_usb_rx_buf_put(usb_dev, usb_buf);
                aicwf_usb_rx_submit_all_urb_(usb_dev);
                return;
...
static int aicwf_usb_chipmatch(struct aic_usb_dev *usb_dev, u16_l vid, u16_l pid){
	...
	if(pid == USB_PRODUCT_ID_AIC8801){
        ...
        return 0;
    }else if(pid == USB_PRODUCT_ID_AIC8800D81 || pid == USB_PRODUCT_ID_AIC8800D41 || pid == USB_PRODUCT_ID_AIC8800D83){

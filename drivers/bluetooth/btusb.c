...
/* Bounded late trace for the F1C200S RTL8723BU HCI receive failure. */
static unsigned int btusb_rtl8723bu_late_event_trace_count;
...
static void btusb_intr_complete(struct urb *urb)
{
	...
	if (le16_to_cpu(data->udev->descriptor.idVendor) == 0x0bda &&
	    le16_to_cpu(data->udev->descriptor.idProduct) == 0xb720 &&
	    time_after(jiffies, 25UL * HZ) &&
	    btusb_rtl8723bu_late_event_trace_count++ < 128)
		bt_dev_info(hdev, "rtl8723bu intr status %d len %d data %*ph",
			    urb->status, urb->actual_length,
			    min_t(unsigned int, urb->actual_length, 8),
			    urb->transfer_buffer);

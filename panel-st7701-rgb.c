...
/*
 * Sitronix ST7701/ST7701S RGB panel bootloader helper for U-Boot.
 *
 * This is meant to be dropped into U-Boot's panel/video tree.
 * It initializes the panel over SPI and lets board code mark the
 * handoff in the DT passed to Linux with sitronix,bootloader-initialized.
 */
...
bool st7701_rgb_bootloader_initialized;
...
static int st7701_rgb_get_display_timing(struct udevice *dev,
					 struct display_timing *timings)
{
	return dev_decode_display_timing(dev, 0, timings);
}
...
static int st7701_rgb_init_panel(struct udevice *dev)
{
	...
	if (!priv->has_dc) {
		ret = spi_set_wordlen(priv->slave, 9);
		if (ret)
...
static const struct panel_ops st7701_rgb_ops = {
	.get_display_timing = st7701_rgb_get_display_timing,
};
...
U_BOOT_DRIVER(st7701_rgb_panel) = {
	...
	.priv_auto = sizeof(struct st7701_rgb_priv),
};

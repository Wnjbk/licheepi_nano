...
STOP_BLUETOOTH=${STOP_BLUETOOTH:-1}
STOP_DBUS=${STOP_DBUS:-1}
DROP_PAGE_CACHE=${DROP_PAGE_CACHE:-1}
START_MEM_WATCH=${START_MEM_WATCH:-0}
START_DMESG_WATCH=${START_DMESG_WATCH:-0}
...
ensure_aic_iface(){
 ...
 log "ERROR: $IFACE not registered; run /root/aic_miracast/aic8800_register_wlan1.sh start first"
 return 1
}
write_watcher(){
...
CDUMP=/root/aic_miracast/miracast_sink_dump
...
dhcp_count(){ awk '/(DISCOVER|REQUEST|ACK)/ && /ip=192\.168\.49\./ {c++} END{print c+0}' "$LOG" 2>/dev/null; }
dhcp_ip(){ grep -E 'ACK|REQUEST|DISCOVER' "$LOG" 2>/dev/null | sed -n 's/.*ip=\(192\.168\.49\.[0-9][0-9]*\).*/\1/p' | tail -1; }
...
while true; do
  ...
  if [ "$cur_dhcp" -gt "$LAST_DHCP_LINE" ]; then
    ...
    if [ "$CAPTURE_MODE" = 'null' ]; then out=/dev/null; else stamp=$(date +%Y%m%d_%H%M%S 2>/dev/null || echo cast); out="$OUT_DIR/miracast_$stamp.h264"; fi
    ...
    if [ "$cur_to" -gt "$LAST_WPS_TIMEOUT" ]; then LAST_WPS_TIMEOUT=$cur_to; logw 'wps timeout observed; wait for phone pbc_req'; fi
    sleep 1
  fi
done
EOF
...
if [ "$LOWMEM_TUNE" = '1' ]; then
  echo 4096 >/proc/sys/vm/min_free_kbytes 2>/dev/null || true
  log "lowmem tune: min_free_kbytes=$(cat /proc/sys/vm/min_free_kbytes 2>/dev/null)"
fi
if [ "$STOP_GMENU" = '1' ]; then
  killall gmenu2x 2>/dev/null || true
  killall run_gmenu2x.sh 2>/dev/null || true
  log 'stopped gmenu2x for Miracast memory headroom'
fi
if [ "$STOP_BLUETOOTH" = '1' ]; then
  killall bluetoothd 2>/dev/null || true
  killall btmon 2>/dev/null || true
  log 'stopped bluetooth userspace for Miracast memory headroom'
fi
if [ "$STOP_DBUS" = '1' ]; then
  killall dbus-daemon 2>/dev/null || true
  log 'stopped dbus-daemon for Miracast memory headroom'
fi
if [ "$DROP_PAGE_CACHE" = '1' ]; then
  sync 2>/dev/null || true
  echo 3 >/proc/sys/vm/drop_caches 2>/dev/null || true
  log 'dropped page cache before Miracast'
fi
...
[ "$START_DMESG_WATCH" = "1" ] && start_dmesg_watch || true
[ "$START_MEM_WATCH" = "1" ] && start_mem_watch || true

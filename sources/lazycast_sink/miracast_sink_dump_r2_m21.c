...
static int body_has_param(const char *body, const char *name)
{
    size_t nlen;
    const char *p;
    ...
    while ((p = strstr(p, name)) != NULL) {
        int left_ok = (p == body) || p[-1] == '\n' || p[-1] == '\r';
        char c = p[nlen];
        int right_ok = c == '\0' || c == '\r' || c == '\n' || c == ' ' || c == ':';
        if (left_ok && right_ok)
            return 1;
        p += nlen;
    }
...
static void send_rtsp_keepalive(int fd, const char *source_ip,
                                const char *session, int *cseq)
{
    (*cseq)++;
    fprintf(stderr, "send RTSP keepalive GET_PARAMETER cseq=%d\n", *cseq);
    sendf(fd, "GET_PARAMETER rtsp://%s/wfd1.0/streamid=0 RTSP/1.0\r\n"
              "CSeq: %d\r\n"
              "Session: %s\r\n\r\n",
          source_ip, *cseq, session);
}
...
int main(int argc, char **argv)
{
    ...
    int rcvbuf = 512 * 1024;
    setsockopt(rtp, SOL_SOCKET, SO_RCVBUF, &rcvbuf, sizeof(rcvbuf));
    ...
    snprintf(m3_body, sizeof(m3_body),
        ...
        body_has_param(buf, "wfd2_audio_codecs") ? "wfd2_audio_codecs: LPCM 000001ff 00\r\n" : "",
        body_has_param(buf, "wfd2_video_formats") ? "wfd2_video_formats: 40 01 04 0080 000001ffbdeb 000155557fff 000000000fff 10 0000 001f 11, 01 01 0080 000001ffbdeb 0001555557ff 000000000fff 10 0000 001f 11 00\r\n" : "",
        body_has_param(buf, "wfd2_video_stream_control") ? "wfd2_video_stream_control: 0f 0f\r\n" : "",
        body_has_param(buf, "wfd2_rotation_capability") ? "wfd2_rotation_capability: supported\r\n" : "");
    ...
    if (getenv("WFD_R2_M21_ROTATE")) {
        const char *m21_body = "wfd2_rotation_control: Landscape_to_Portrait\r\n";
        fprintf(stderr, "send M21 rotation request: %s", m21_body);
        sendf(rtsp, "SET_PARAMETER rtsp://%s/wfd1.0/streamid=0 RTSP/1.0\r\n"
                    "CSeq: 21\r\n"
                    "Session: %s\r\n"
                    "Content-Type: text/parameters\r\n"
                    "Content-Length: %zu\r\n\r\n%s",
              source_ip, session, strlen(m21_body), m21_body);
        read_rtsp_message(rtsp, &rr, buf, sizeof(buf), "---M21<---");
        if (strstr(buf, "RTSP/1.0 200 OK"))
            fprintf(stderr, "M21 rotation request accepted\n");
        else
            fprintf(stderr, "M21 rotation request unexpected response\n");
    }
    ...
    while (!g_stop) {
        ...
        if (rc == 0) {
            ...
            if (now - last_packet > 10 && now - last_keepalive >= 10) {
                send_rtsp_keepalive(rtsp, source_ip, session, &local_cseq);
                last_keepalive = now;
            }
            continue;
        }
        ...
        if (FD_ISSET(rtsp, &rfds)) {
            if (recv_rtsp_available(rtsp, &rr) < 0) {
                fprintf(stderr, "RTSP peer closed control connection\n");
                break;
            }
            while (pop_rtsp_message(&rr, buf, sizeof(buf), "<---RTSP---")) {
                if (strstr(buf, "wfd_trigger_method: TEARDOWN")) {
        ...
        if (FD_ISSET(rtp, &rfds)) {
            for (;;) {
                ...
                if (n == 0)
                    break;
                die("recv RTP failed: %s", strerror(errno));
            }
        }
    }

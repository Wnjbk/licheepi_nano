...
LOG="${LOG:-/tmp/wpa24_8723bu_wfd_listen.log}"
...
"$WPA_DIR/wpa_cli" -p "$CTRL_DIR" -i "$IFACE" p2p_find type=social >/dev/null || true
"$WPA_DIR/wpa_cli" -p "$CTRL_DIR" -i "$IFACE" p2p_listen 600 >/dev/null || true

echo "wpa_supplicant 2.4 p2p_concurrent=0 WFD listen started"
"$WPA_DIR/wpa_cli" -p "$CTRL_DIR" -i "$IFACE" status || true
"$WPA_DIR/wpa_cli" -p "$CTRL_DIR" -i "$IFACE" get wifi_display || true
"$WPA_DIR/wpa_cli" -p "$CTRL_DIR" -i "$IFACE" p2p_peers || true
iw dev
tail -80 "$LOG" || true

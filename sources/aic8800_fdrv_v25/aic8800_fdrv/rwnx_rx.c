...
struct rwnx_vif *rwnx_rx_get_vif(struct rwnx_hw *rwnx_hw, int vif_idx)
{
    ...
    if (vif_idx >= NX_VIRT_DEV_MAX) {
        if (printk_ratelimit())
            printk("aicwf: drop rx frame invalid vif_idx=%d\n", vif_idx);
        return NULL;
    }
    ...
    if (!rwnx_vif || !rwnx_vif->up) {
        if (printk_ratelimit())
            printk("aicwf: drop rx frame inactive vif_idx=%d vif=%p up=%d\n",
                   vif_idx, rwnx_vif, rwnx_vif ? rwnx_vif->up : 0);
        return NULL;
    }
                                        ...
                                        struct sk_buff_head *list)
{
    ...
    //if (is_amsdu) 
    {
        ...
        while (skb->len > 16) {
            ...
            if (sublen < 6 || sublen > 2304) {
                if (printk_ratelimit())
                    printk("aicwf: drop bad amsdu sublen=%u skb_len=%u\n", sublen, skb->len);
                break;
            }
            ...
            if (!rwnx_vif) {
                if (printk_ratelimit())
                    printk("aicwf: drop amsdu subframe no active vif (%d), skb_len=%u\n", vif_idx, skb->len);
...
check_len_update:
    ...
    if (status & RX_STAT_FORWARD) {
        ...
        if (!hw_rxhdr->flags_is_80211_mpdu &&
            hw_rxhdr->flags_vif_idx >= NX_VIRT_DEV_MAX) {
            if (printk_ratelimit())
                printk("aicwf: drop rx data bad vif_idx=%u sta_idx=%u dst_idx=%u len=%u\n",
                       hw_rxhdr->flags_vif_idx, hw_rxhdr->flags_sta_idx,
                       hw_rxhdr->flags_dst_idx, skb->len);
            dev_kfree_skb(skb);
            goto end;
        }
...
//Check NETGEAR R7000 router's AMSDU packet format for compliance. end
			...
			if (hw_rxhdr->flags_dst_idx != RWNX_INVALID_STA) {
				if (hw_rxhdr->flags_dst_idx >= (NX_REMOTE_STA_MAX + NX_VIRT_DEV_MAX)) {
					if (printk_ratelimit())
						printk("aicwf: drop rx data bad dst_idx=%u len=%u\n",
							   hw_rxhdr->flags_dst_idx, skb->len);
					dev_kfree_skb(skb);
					goto end;
				}
        ...
        if (hw_rxhdr->flags_is_80211_mpdu) {
            ...
            if (hw_rxhdr->flags_sta_idx != RWNX_INVALID_STA) {
                ...
                if (hw_rxhdr->flags_sta_idx >= (NX_REMOTE_STA_MAX + NX_VIRT_DEV_MAX)) {
                    if (printk_ratelimit())
                        printk("aicwf: drop rx data bad sta_idx=%u len=%u\n",
                               hw_rxhdr->flags_sta_idx, skb->len);
                    dev_kfree_skb(skb);
                    goto end;
                }
                ...
                if (sta->vlan_idx != rwnx_vif->vif_index) {
                    if (sta->vlan_idx >= NX_VIRT_DEV_MAX) {
                        if (printk_ratelimit())
                            printk("aicwf: drop rx data bad vlan_idx=%u sta_idx=%u len=%u\n",
                                   sta->vlan_idx, hw_rxhdr->flags_sta_idx, skb->len);
                        dev_kfree_skb(skb);
                        goto end;
                    }
                ...
                if (hw_rxhdr->flags_is_4addr && !rwnx_vif->use_4addr) {
                    /* Ignore unexpected 4addr notification on this tiny target. */
                }
            }

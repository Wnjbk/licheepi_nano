...
static bool aicwf_rx_lowmem(void)
{
    struct sysinfo i;
    ...
    while (1) {
        ...
        while(aicwf_another_ptk(skb)) {
            ...
            if((skb->data[2] & SDIO_TYPE_CFG) != SDIO_TYPE_CFG) { // type : data
                ...
                skb_inblock = __dev_alloc_skb(aggr_len + CCMP_OR_WEP_INFO, GFP_ATOMIC | __GFP_NOWARN);
                if(skb_inblock == NULL){
                    txrx_err("no more space! skip\n");
                    skb_pull(skb, adjust_len);
                    continue;
        ...
        while (1) {
            ...
            while(aicwf_another_ptk_1(buffer)) {
                ...
                if (pkt_len > buffer->len) {
                    if (printk_ratelimit())
                        printk("aicwf: drop bad rx buffer pkt_len=%d buffer_len=%d\n", pkt_len, buffer->len);
...
#endif
            if((data[2] & USB_TYPE_CFG) != USB_TYPE_CFG) { // type : data
                skb_inblock = __dev_alloc_skb(pkt_len + RX_HWHRD_LEN + CCMP_OR_WEP_INFO, GFP_ATOMIC | __GFP_NOWARN);
                if (skb_inblock == NULL) {
                    txrx_err("no more space! skip\n");
                    aicwf_prealloc_rxbuff_free(buffer, &rx_priv->rxbuff_lock);
                    atomic_dec(&rx_priv->rx_cnt);
    ...
    if(aicwf_usb_rx_aggr){
        while (1) {
            ...
            while(aicwf_another_ptk(skb)) {
                ...
                if (cnt > 16) {
                    if (printk_ratelimit())
                        printk("aicwf: drop rx aggregate over subframe budget cnt=%u len=%u\n", cnt, skb->len);
                    break;
                }
                data = skb->data;
                pkt_len = (*skb->data | (*(skb->data + 1) << 8));
                ...
                if (pkt_len > skb->len) {
                    if (printk_ratelimit())
                        printk("aicwf: drop bad rx skb pkt_len=%d skb_len=%d\n", pkt_len, skb->len);
                    //dev_kfree_skb(skb);
                    //atomic_dec(&rx_priv->rx_cnt);
                    //continue;
                    break;
                ...
                if((skb->data[2] & USB_TYPE_CFG) != USB_TYPE_CFG) { // type : data
                    if (aicwf_rx_lowmem()) {
                        if (printk_ratelimit())
                            printk("aicwf: drop rx data under skb pressure len=%u remain=%u\n", pkt_len, skb->len);
                        break;
                    }
                    aggr_len = pkt_len + RX_HWHRD_LEN;
                    adjust_len = aggr_len;
                    skb_inblock = __dev_alloc_skb(aggr_len + CCMP_OR_WEP_INFO, GFP_ATOMIC | __GFP_NOWARN);//8 is for ccmp mic or wep icv
                    if(skb_inblock == NULL){
                        if (printk_ratelimit())
                            printk("aicwf: drop rx aggregate alloc fail len=%u remain=%u\n", aggr_len, skb->len);
                        break;
                    }

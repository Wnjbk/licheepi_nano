			   ...
			   struct usb_port *port_dev)
{
	...
	if (hcd) {
		if (udev->bus->controller && udev->bus->controller->driver)
			driver_name = udev->bus->controller->driver->name;
		else if (udev->bus->sysdev && udev->bus->sysdev->driver)
...
#define GET_DESCRIPTOR_BUFSIZE	64
			...
			if (r) {
				if (driver_name && strstr(driver_name, "musb")) {
					u16 dbg_portstatus = 0;
					u16 dbg_portchange = 0;
					int dbg_ret;

					dbg_ret = hub_port_status(hub, port1,
							&dbg_portstatus,
							&dbg_portchange);
					dev_info(&udev->dev,
						 "musb descriptor fail retry=%d op=%d r=%d speed=%d timeout=%d port_ret=%d status=%04x change=%04x\n",
						 retries, operations, r,
						 udev->speed,
						 initial_descriptor_timeout,
						 dbg_ret, dbg_portstatus,
						 dbg_portchange);
				}
				if (r != -ENODEV)

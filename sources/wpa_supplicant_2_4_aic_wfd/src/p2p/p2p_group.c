			  ...
			  const u8 *ie, size_t len)
{
	...
	if (group->num_members == group->cfg->max_clients)
		group->beacon_update = 1;
	/* F1C200S AIC8800 board: dynamic GO IE/beacon update during assoc
	 * crashes the USB command queue before NL80211_CMD_NEW_STATION.
	 * Keep association state, but defer/skip this SET_BEACON update. */
	p2p_dbg(group->p2p, "F1C200S AIC: skip dynamic GO IE update on assoc");
...
void p2p_group_notif_disassoc(struct p2p_group *group, const u8 *addr)
{
	if (p2p_group_remove_member(group, addr)) {
		...
		if (group->num_members == group->cfg->max_clients - 1)
			group->beacon_update = 1;
		p2p_dbg(group->p2p, "F1C200S AIC: skip dynamic GO IE update on disassoc");
